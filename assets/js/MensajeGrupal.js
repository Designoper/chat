...
export default class MensajeGrupal extends Usuario {
	...
	async getMensajesGrupales() {
		...
		this.printMensajesGrupales(response);
	}

	mensajesGrupalesTemplate(fetchedMensajes) {
	...
	printMensajesGrupales(mensajes) {
		const output = document.querySelector('output');
		const content = this.mensajesGrupalesTemplate(mensajes.content);
